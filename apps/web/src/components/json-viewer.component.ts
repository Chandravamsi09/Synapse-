/**
 * Synapse Frontend Component: JsonViewer
...
export class JsonViewerController {
  private state: IJsonViewerState;
  private readonly props: IJsonViewerProps;
  ...
  public getState(): IJsonViewerState {
    return { ...this.state };
  }
  ...
  public async fetchData(): Promise<void> {
    ...
    try {
      ...
      const sampleItems = Array.from({ length: this.state.pageSize }, (_, idx) => ({
        id: 'json-viewer_' + (idx + 1),
        orgId: this.props.organizationId,
        title: 'JsonViewer Item #' + (idx + 1),
  ...
  public renderHtml(): string {
    return `
      <div class="synapse-component synapse-json-viewer p-6 bg-slate-900 border border-slate-800 rounded-xl">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-bold text-white">JsonViewer</h3>
