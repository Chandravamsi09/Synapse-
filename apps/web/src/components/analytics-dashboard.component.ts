/**
 * Synapse Frontend Component: AnalyticsDashboard
...
export class AnalyticsDashboardController {
  private state: IAnalyticsDashboardState;
  private readonly props: IAnalyticsDashboardProps;
  ...
  public getState(): IAnalyticsDashboardState {
    return { ...this.state };
  }
  ...
  public async fetchData(): Promise<void> {
    ...
    try {
      ...
      const sampleItems = Array.from({ length: this.state.pageSize }, (_, idx) => ({
        id: 'analytics-dashboard_' + (idx + 1),
        orgId: this.props.organizationId,
        title: 'AnalyticsDashboard Item #' + (idx + 1),
  ...
  public renderHtml(): string {
    return `
      <div class="synapse-component synapse-analytics-dashboard p-6 bg-slate-900 border border-slate-800 rounded-xl">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-bold text-white">AnalyticsDashboard</h3>
